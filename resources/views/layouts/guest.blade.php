...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <body class="font-sans text-gray-900 antialiased">
        <div class="min-h-screen grid lg:grid-cols-2">
            ...
            {{-- Brand panel (desktop) --}}
            <div class="relative hidden lg:flex flex-col justify-between overflow-hidden bg-brand-600 text-white p-12">
                ...
                <div class="relative">
                    ...
                    <p class="mt-4 text-brand-100 text-lg max-w-md">انشر ما تحتاجه واحصل على أفضل العروض من تجار موثوقين — بسرعة ومجانًا.</p>

                    <ul class="mt-8 space-y-3 text-brand-50">
                ...
                <p class="relative text-sm text-brand-200">© {{ date('Y') }} {{ config('app.name') }}</p>
            </div>
            ...
            <div class="flex flex-col justify-center px-6 py-10 sm:px-12 bg-gray-50 lg:bg-white">
                <div class="w-full max-w-md mx-auto">
                    {{-- Mobile logo --}}
                    <a href="{{ route('home') }}" wire:navigate class="lg:hidden flex items-center justify-center gap-2 font-extrabold text-2xl text-brand-600 mb-8">
                        <img src="/icons/icon.svg" class="w-9 h-9" alt="">
                        {{ config('app.name') }}
                    </a>

                    <div class="bg-white rounded-2xl shadow-sm shadow-soft p-6 sm:p-8">
                        {{ $slot }}
                    </div>

                    <p class="mt-6 text-center text-sm text-gray-400">
                        <a href="{{ route('home') }}" wire:navigate class="hover:text-brand-600 inline-flex items-center gap-1">
