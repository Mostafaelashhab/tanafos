    ...
    <div class="max-w-2xl mx-auto px-4 py-5 space-y-6">
        ...
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="{{ route('merchant.billing') }}" wire:navigate
               class="flex items-center gap-2 bg-white ring-1 ring-gray-100 rounded-full ps-3 pe-1 py-1">
                <span class="text-sm font-bold text-indigo-600">{{ $profile?->credits_balance ?? 0 }}</span>
                <span class="text-xs text-gray-400">{{ __('credits') }}</span>
                <span class="w-7 h-7 rounded-full bg-indigo-600 text-white flex items-center justify-center">
        ...
        <section>
            <div class="flex items-center justify-between mb-3">
                ...
                <a href="{{ route('merchant.leads.index') }}" wire:navigate class="text-sm text-indigo-600">{{ __('View all') }}</a>
            </div>

            @forelse ($leads as $lead)
                <a href="{{ route('merchant.leads.show', $lead) }}" wire:navigate
                   class="block bg-white rounded-2xl ring-1 ring-gray-100 p-4 mb-3 active:bg-gray-50 transition">
                    <div class="flex items-center justify-between gap-3">
                        <div class="min-w-0">
                        ...
                        <div class="text-end shrink-0">
                            <div class="text-lg font-extrabold text-indigo-600">{{ $lead->quality_score }}%</div>
