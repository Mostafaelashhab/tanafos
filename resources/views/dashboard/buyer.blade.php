<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto px-4 py-5 space-y-6">
        ...
        <div class="flex items-center gap-3">
            <span class="w-12 h-12 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center font-extrabold text-lg shrink-0">
        ...
        <a href="{{ route('requests.create') }}" wire:navigate
           class="block relative overflow-hidden rounded-3xl bg-brand-600 text-white p-6 active:scale-[.99] transition">
            ...
            <div class="relative">
                ...
                <p class="mt-1 text-brand-100 text-sm">اكتب طلبك ودع التجار يتنافسون عليك.</p>
                <span class="mt-4 inline-flex items-center gap-2 bg-white text-brand-700 font-semibold rounded-full px-5 py-2.5 text-sm">
        ...
        <section>
            <div class="flex items-center justify-between mb-3">
                ...
                <a href="{{ route('requests.index') }}" wire:navigate class="text-sm text-brand-600">{{ __('View all') }}</a>
            </div>

            @php($chips = [['bg-brand-50','text-brand-600'],['bg-rose-50','text-rose-500'],['bg-amber-50','text-amber-600'],['bg-sky-50','text-sky-600'],['bg-emerald-50','text-emerald-600']])
            @forelse ($active as $request)
                @php($chip = $chips[$loop->index % count($chips)])
                <a href="{{ route('requests.show', $request) }}" wire:navigate
                   class="flex items-center gap-3 bg-white rounded-2xl shadow-soft p-4 mb-3 active:scale-[.99] transition">
                    <span class="w-11 h-11 rounded-2xl {{ $chip[0] }} {{ $chip[1] }} flex items-center justify-center shrink-0">
                    ...
                    @if ($request->offers_count)
                        <span class="shrink-0 inline-flex items-center gap-1 bg-accent-100 text-accent-700 text-xs font-bold rounded-full px-2.5 py-1">
            ...
            <section>
                ...
                @foreach ($recentOffers as $offer)
                    <a href="{{ route('requests.show', $offer->request) }}" wire:navigate
                       class="flex items-center gap-3 bg-white rounded-2xl shadow-soft p-4 mb-3 active:bg-gray-50 transition">
                        ...
                        <span class="shrink-0 font-bold text-brand-600">{{ $offer->price }} <span class="text-xs font-medium">{{ __('EGP') }}</span></span>
                    </a>
                @endforeach
            </section>
        @endif
